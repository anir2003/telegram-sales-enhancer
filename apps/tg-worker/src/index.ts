...
function buildSyncedMessageMediaMetadata(message: any): Record<string, unknown> {
  const media = message?.media;
  if (!media) return { media: false };
  const mediaClass = getClassName(media);

  if (mediaClass === 'MessageMediaPhoto') {
    return { media: true, media_kind: 'photo', file_name: 'Photo', mime_type: 'image/jpeg' };
  }

  if (mediaClass === 'MessageMediaDocument') {
    const document = media.document;
    const attributes: any[] = Array.isArray(document?.attributes) ? document.attributes : [];
    const mimeType = typeof document?.mimeType === 'string' ? document.mimeType : null;
    const fileName = attributes.find((attribute) => getClassName(attribute) === 'DocumentAttributeFilename')?.fileName ?? null;
    const sticker = attributes.find((attribute) => getClassName(attribute) === 'DocumentAttributeSticker') ?? null;
    const sized = attributes.find((attribute) => ['DocumentAttributeImageSize', 'DocumentAttributeVideo'].includes(getClassName(attribute))) ?? null;
    const audio = attributes.find((attribute) => getClassName(attribute) === 'DocumentAttributeAudio') ?? null;
    const animated = attributes.some((attribute) => getClassName(attribute) === 'DocumentAttributeAnimated');
    ...
    return compactMetadata({
      ...
      sticker_animated: isSticker && (mimeType === 'application/x-tgsticker' || animated),
...
function getPreview(message: any) {
  ...
  const metadata = buildSyncedMessageMediaMetadata(message);
    ...
    for (const dialog of dialogs as any[]) {
      ...
      const rows = (messages as any[]).reverse().map((message) => ({
        ...
        metadata: {
          ...
          ...buildSyncedMessageMediaMetadata(message),
