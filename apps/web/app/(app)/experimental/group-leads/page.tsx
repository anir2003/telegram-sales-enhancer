...
function exportCsv(results: TgGroupLeadResultRecord[], job: TgGroupLeadScrapeJobRecord | null) {
  const header = ['Name', 'Username', 'Bio', 'Premium', 'Profile Picture'];
...
function ResultsPanel({
  ...
  const save = async () => {
    ...
    try {
      ...
      setStatus(`${result.inserted} added, ${result.updated} updated.`);
      onSaved();
    } catch (error) {
  ...
  return (
    <section className="tg-console-panel group-leads-results">
      ...
      <div className="group-leads-save-row">
        <input className="input" placeholder="Tag for saved leads" value={tag} onChange={(event) => setTag(event.target.value)} />
        <button className="btn" disabled={busy || !job || !tag.trim() || !results.length} onClick={save}>
          {busy ? 'Saving...' : 'Save as leads'}
        </button>
      </div>
